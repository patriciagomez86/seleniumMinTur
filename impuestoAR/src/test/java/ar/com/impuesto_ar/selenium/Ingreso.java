...
public class Ingreso {
	...
	@FindBy(xpath = "//div[@class='text-center alert alert-info']")	
	private WebElement msjLogout;	
	
	


	public Ingreso(WebDriver driver, String url) {
		this.driver = driver;
	...
	public Ingreso aniadirUsuario(String nombreDeUsuario) {
		try {
	...
	public Ingreso aniadirContrasenia(String contrasenia) {
		try {
	...
	public Ingreso login(String nombreDeUsuario, String contrasenia) throws InterruptedException {
		try { 
	...
	public Ingreso clickBtnIngreso() throws InterruptedException {
		try { 						 			
			wait.until(ExpectedConditions.elementToBeClickable(btnLoginBoton));			 
			Thread.sleep(1000);
			this.btnLoginBoton.click();
	...
	public boolean esVisibleMsjUsrLogout(String msjValidacion)  throws InterruptedException  {
		try {
			wait.until(ExpectedConditions.elementToBeClickable(msjLogout));			
			if (msjLogout.isDisplayed()) {				
				if (msjLogout.getText().equals(msjValidacion)) {
					return true;
				}			
			}			
		} catch (Exception e) {
